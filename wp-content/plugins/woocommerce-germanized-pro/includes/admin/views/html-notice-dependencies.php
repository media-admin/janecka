...
<div id="message" class="error woocommerce-gzd-message wc-connect">
    <h3><?php _e( 'WooCommerce missing or outdated', 'woocommerce-germanized-pro' ); ?></h3>
    <p>
        ...
        elseif ( $dependencies->is_woocommerce_outdated() ) :
			...
			printf( esc_html__( '%1$sGermanized Pro is inactive.%2$s This version of Germanized Pro requires WooCommerce %3$s or newer. Please %4$supdate WooCommerce to version %3$s or newer &raquo;%5$s', 'woocommerce-germanized-pro' ), '<strong>', '</strong>', $dependencies->get_wc_min_version_required(), '<a href="' . esc_url( admin_url( 'plugins.php' ) ) . '">', '</a>' );
		endif; ?>
    </p>
</div>