...
jQuery(
	function ($) {
		...
		if ( $( '.single-product.woocommerce div.product div.images ul.yith_magnifier_gallery' ).length ) {
			...
			function yith_wc_zm_carousel(){
				...
				$( ".single-product.woocommerce div.product div.images div.thumbnails" ).css( 'width', $( ".single-product.woocommerce div.product div.images" ).width() );
    ...
    $(document).on('mouseover', '.yith_expand', function (e) {
      $( this ).css( 'background-color', ywzm_data.lighbox_background_colors['background_hover'] );
      $( this ).find( '.expand-button-hidden path').css( 'fill', ywzm_data.lighbox_icon_colors['icon_hover'] );
    });

    $(document).on('mouseout', '.yith_expand', function (e) {
      $( this ).css( 'background-color', ywzm_data.lighbox_background_colors['background'] );
      $( this ).find( '.expand-button-hidden path').css( 'fill', ywzm_data.lighbox_icon_colors['icon'] );
    });

    $(document).on('mouseover', '.yith_zoom_icon', function (e) {
      $( this ).css( 'background-color', ywzm_data.zoom_background_colors['background_hover'] );
      $( this ).find( '.zoom-button-hidden path').css( 'fill', ywzm_data.zoom_icon_colors['icon_hover'] );
    });

    $(document).on('mouseout', '.yith_zoom_icon', function (e) {
      $( this ).css( 'background-color', ywzm_data.zoom_background_colors['background'] );
      $( this ).find( '.zoom-button-hidden path').css( 'fill', ywzm_data.zoom_icon_colors['icon'] );
    });
    ...
    // Manage the "change image on" option
    if ( ywzm_data.change_image_on === 'hover' ){
      $(document).on('mouseover', '.yith_magnifier_gallery a.yith_magnifier_thumbnail', function (e) {
        $( this ).click();
      });
    }

      if ( ywzm_data.thumbnails_opacity === 'yes' && ywzm_data.thumbnails_grey_scale === 'no' ){
        $(document).on('mouseover', '.yith_magnifier_gallery .yith_magnifier_thumbnail:not(.active-thumbnail) img', function (e) {
          $( this ).css( 'opacity', '1' );
        });

        $(document).on('mouseout', '.yith_magnifier_gallery .yith_magnifier_thumbnail:not(.active-thumbnail) img', function (e) {
          $( this ).css( 'opacity', '0.5' );
        });
      }

      if ( ywzm_data.thumbnails_grey_scale === 'yes' ){
        $(document).on('mouseover', '.yith_magnifier_gallery .yith_magnifier_thumbnail:not(.active-thumbnail) img', function (e) {
          $( this ).css( 'opacity', '1' );
